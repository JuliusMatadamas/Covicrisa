...
if ($_SERVER['REQUEST_METHOD'] == "POST")
{
    ...
    try {
        ...
        $sql = "UPDATE productos SET clave = '$clave', nombre = '$nombre', descripcion = '$descripcion', presentacion = '$presentacion', precio = '$precio', imagen = '$imagen', proveedor_id = $proveedor_id WHERE id = $id";
        $count = $db->exec($sql);
        ...
        if ($count)
        {
            ...
            if (move_uploaded_file($_FILES["imagen"]['tmp_name'], "$dir/$name"))
            {
                ...
                echo json_encode(['message' => 'El producto fue actualizado']);
                exit;
            }
            else{
                header("Access-Control-Allow-Origin: *");
                header("content-type:application/json");
                http_response_code(200);
                echo json_encode(['message' => 'El producto fue actualizado pero no se pudo guardar la imagen en el servidor.']);
                exit;
            }
        }
        else
